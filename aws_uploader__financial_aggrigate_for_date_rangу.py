...
query = """ 
with active_darkstores as (

SELECT
  FORMAT_TIMESTAMP('%Y.%m.%d', max( mart_orders.created_at)) AS date,
  ...
  -- SUM(mart_orders.price) AS total_price,
FROM
  `@bq_table_addres@` AS mart_orders

WHERE TIMESTAMP_TRUNC(mart_orders.created_at, day) = '@dt@'
  AND mart_orders.order_delivered_dttm IS NOT NULL
...
SELECT
  *,
  ...
  -- gmv - total_price as platform_funded_incentives,
  from active_darkstores
"""
...
if __name__ == '__main__':
    ...
    # end_date = '20260317'    # End date in YYYYMMDD format
